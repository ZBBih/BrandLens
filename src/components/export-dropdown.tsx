...
export function ExportDropdown({ report, variant = 'full' }: ExportDropdownProps) {
  ...
  const generateBrandBoard = async () => {
    ...
    try {
      ...
      // Create a temporary container for the brand board
      ...
      container.innerHTML = `
        <div style="margin-bottom: 40px;">
          <h1 style="font-size: 48px; font-weight: 700; color: #1e293b; margin: 0;">${report.brandName}</h1>
          <p style="font-size: 18px; color: #64748b; margin-top: 8px;">Brand Guidelines</p>
        </div>

        <div style="display: grid; grid-template-columns: 1fr 1fr; gap: 40px;">
          <div>
            <h2 style="font-size: 14px; text-transform: uppercase; letter-spacing: 0.1em; color: #94a3b8; margin-bottom: 16px;">Color Palette</h2>
            <div style="display: flex; gap: 12px; flex-wrap: wrap;">
              ${report.colors.colors.slice(0, 6).map(color => `
                <div style="text-align: center;">
                  <div style="width: 80px; height: 80px; background-color: ${color.hex}; border-radius: 12px; margin-bottom: 8px;"></div>
                  <p style="font-size: 12px; font-weight: 600; color: #1e293b; margin: 0;">${color.hex.toUpperCase()}</p>
                  <p style="font-size: 10px; color: #94a3b8; margin: 4px 0 0 0;">${color.role}</p>
                </div>
              `).join('')}
            </div>
          </div>
          ...
          <div>
            <h2 style="font-size: 14px; text-transform: uppercase; letter-spacing: 0.1em; color: #94a3b8; margin-bottom: 16px;">Typography</h2>
            <div style="space-y: 16px;">
              ${report.typography.fonts.slice(0, 3).map(font => `
                <div style="margin-bottom: 16px;">
                  <p style="font-size: 32px; font-weight: 700; color: #1e293b; margin: 0; font-family: '${font.name}', sans-serif;">AaBbCc</p>
        ...
        <div style="margin-top: 40px; padding-top: 40px; border-top: 1px solid #e2e8f0;">
          <h2 style="font-size: 14px; text-transform: uppercase; letter-spacing: 0.1em; color: #94a3b8; margin-bottom: 16px;">Brand Voice</h2>
          <div style="display: flex; gap: 12px; flex-wrap: wrap;">
            ${report.tone.traits.slice(0, 5).map(trait => `
              <span style="padding: 8px 16px; background-color: ${primaryColor}15; color: ${primaryColor}; border-radius: 20px; font-size: 14px; font-weight: 500;">${trait}</span>
            `).join('')}
          </div>
        </div>

        <div style="margin-top: 40px; text-align: center; color: #94a3b8; font-size: 12px;">
          Generated by BrandLens · ${new Date().toLocaleDateString()}
        </div>
      `
      ...
      link.download = `${report.brandName.toLowerCase().replace(/\s+/g, '-')}-brand-board.png`
      link.href = canvas.toDataURL('image/png')
      link.click()
    } catch (error) {
      console.error('Failed to generate brand board:', error)
    } finally {
      setGenerating(false)
    }
  }
  ...
  const fullOptions: ExportOption[] = [
    ...
    {
      label: generating ? 'Generating...' : 'Download Brand Board (PNG)',
      icon: <ImageIcon />,
      action: generateBrandBoard,
    },
    {
