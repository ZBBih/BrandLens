/**
 * Typography detection from Playwright computed styles and from raw CSS
 * (the legacy and fallback layers of extractTypography).
 */

import type { PageData, ComputedFontInfo, FontSource } from '../../crawler'
import type { FontEntry } from '../types'
import { fontExtras, hasUnresolvedVar, isSystemFont, parseGoogleFontsUrl } from './parse'
 ...
 */
export function extractFromComputedFonts(
  pages: PageData[],
  fontSources: FontSource[]
  ...
  for (const page of pages) {
    ...
    const extras = fontExtras(page)
    if (extras.loadedFonts) {
      loadedFonts = [...new Set([...loadedFonts, ...extras.loadedFonts])]
    }
    if (extras.googleFonts) {
      googleFonts = [...new Set([...googleFonts, ...extras.googleFonts])]
    }
    if (extras.cssVariables) {
      cssVariables = { ...cssVariables, ...extras.cssVariables }
    }
  }
  ...
  for (const page of pages) {
    ...
    const headingFontData = computedFonts.h1 || computedFonts.h2 || computedFonts.h3 || computedFonts.hero
